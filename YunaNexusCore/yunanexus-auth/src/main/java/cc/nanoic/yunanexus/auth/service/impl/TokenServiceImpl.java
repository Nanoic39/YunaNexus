...
public class TokenServiceImpl implements TokenService {
    ...
    public OAuthLoginTokenVO generateToken(OAuthClients client, String subject) {
        ...
        yunaRedisService.set(authRedisKeyPrefixProvider.refreshKey(refreshToken), subject, Duration.ofSeconds(refreshTtl));
        yunaRedisService.set(authRedisKeyPrefixProvider.pairRefreshKey(refreshToken), jti, Duration.ofSeconds(refreshTtl));
        yunaRedisService.set(authRedisKeyPrefixProvider.pairJtiKey(jti), refreshToken, Duration.ofSeconds(refreshTtl));
    ...
    public OAuthLoginTokenVO refreshToken(OAuthClients client, String refreshToken) {
        ...
        String subject = yunaRedisService.get(authRedisKeyPrefixProvider.refreshKey(refreshToken));
        if (!StringUtils.hasText(subject)) {
            return null;
        }

        String oldJti = yunaRedisService.get(authRedisKeyPrefixProvider.pairRefreshKey(refreshToken));
        if (StringUtils.hasText(oldJti)) {
            yunaRedisService.set(authRedisKeyPrefixProvider.blacklistAccessKey(oldJti), "1", Duration.ofSeconds(3600));
            yunaRedisService.delete(authRedisKeyPrefixProvider.pairJtiKey(oldJti));
        }

        yunaRedisService.delete(authRedisKeyPrefixProvider.refreshKey(refreshToken));
        yunaRedisService.delete(authRedisKeyPrefixProvider.pairRefreshKey(refreshToken));
    ...
    public boolean logoutByAccessToken(String accessToken) {
        ...
        yunaRedisService.set(authRedisKeyPrefixProvider.blacklistAccessKey(jti), "1", Duration.ofSeconds(expSeconds - nowSeconds));
        // 找到并删除 refreshToken 防止续签 Token
        String refreshToken = yunaRedisService.get(authRedisKeyPrefixProvider.pairJtiKey(jti));
        if (StringUtils.hasText(refreshToken)) {
            yunaRedisService.delete(authRedisKeyPrefixProvider.refreshKey(refreshToken));
            yunaRedisService.delete(authRedisKeyPrefixProvider.pairRefreshKey(refreshToken));
            yunaRedisService.delete(authRedisKeyPrefixProvider.pairJtiKey(jti));
        }
    ...
    public Map<String, Object> parseAccessToken(String accessToken) {
        ...
        String jti = String.valueOf(jtiObj);
        if (StringUtils.hasText(yunaRedisService.get(authRedisKeyPrefixProvider.blacklistAccessKey(jti)))) {
            return null;
        }
