...
def initByPlain(clientName: str, plainSecret: str, redirectUri: str = "localhost:3000/oauth/callback") -> str | None:
    ...
    # 拼接输出命令
    res = f"INSERT INTO oauth_clients (uuid, client_type, audit_status, audit_opinion, redirect_whitelist, client_secret, client_name, authorized_grant_types, scope, redirect_uri, access_token_validity, refresh_token_validity, auto_approve, status) VALUES ('{uid}', 1, 1, '官方应用【SystemInit】', '{redirectUri}', '{bcryptSecret}', '{clientName}', 'password,refresh_token', 'all', '{redirectUri}', 7200, 604800, 0, 1);"
    return res

if __name__ == "__main__":
    name = input("请输入客户端名称：")
    secret = generateSecret() # 可以自定义密钥长度
    redirect = input(f"请输入回调地址(默认为\"{DEFAULT_REDIRECT_URI}\")：").strip()
    if (not redirect):
