...
export default defineEventHandler(async (event) => {
  ...
  const previewPath = extractCode
    ? `/file/share/preview?shareCode=${encodeURIComponent(String(shareCode))}&extractCode=${encodeURIComponent(String(extractCode))}`
    : `/file/share/preview?shareCode=${encodeURIComponent(String(shareCode))}`;

  const range = getHeader(event, "range");

  const response = await fetch(joinUrl(fileBase, previewPath), {
    method: "GET",
    headers: {
